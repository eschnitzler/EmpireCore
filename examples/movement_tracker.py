...
async def main():
    ...
    async def on_movement_started(event: MovementStartedEvent):
        ...
        logger.info(
            f"New {direction} {event.movement_type_name} detected! "
            f"ID: {event.movement_id}, Units: {event.unit_count}"
        )
    ...
    async def on_movement_arrived(event: MovementArrivedEvent):
        ...
        logger.info(
            f"Movement {event.movement_id} ({event.movement_type_name}) has arrived! "
            f"Target: {event.target_area_id}"
        )
    ...
    try:
        ...
        if not movements:
            ...
            if attacks:
                ...
                for attack in attacks:
                    print(
                        f"  - ID {attack.MID}: {attack.format_time_remaining()} remaining"
                    )
        ...
        async def on_movements_updated(movements):
            ...
            if movements:
                ...
                if MovementHelper.is_attack_imminent(
                    client.movements, threshold_seconds=60
                ):
        ...
        try:
            await client.watch_movements(
                interval=10.0, callback=on_movements_updated, stop_event=stop_event
            )
...
async def demo_movement_queries():
    ...
    # Get by direction
    incoming = client.get_incoming_movements()
    outgoing = client.get_outgoing_movements()
    returning = client.get_returning_movements()

    # Get attacks specifically
    incoming_attacks = client.get_incoming_attacks()

    # Get movements for specific castle
    castle_id = 12345
    to_castle = client.get_movements_to_castle(castle_id)
    from_castle = client.get_movements_from_castle(castle_id)

    # Get specific movement by ID
    movement = client.get_movement(movement_id=67890)
    ...
    attacks = MovementHelper.get_movements_by_type(movements_dict, MovementType.ATTACK)
    transports = MovementHelper.get_movements_by_type(
        movements_dict, MovementType.TRANSPORT
    )
    supports = MovementHelper.get_movements_by_type(
        movements_dict, MovementType.SUPPORT
    )

    # Get movements arriving within 5 minutes
    soon = MovementHelper.get_movements_arriving_within(movements_dict, seconds=300)

    # Get total units across all movements
    total_units = MovementHelper.get_total_units_in_movements(all_movements)

    # Get total resources in transports
    total_resources = MovementHelper.get_total_resources_in_movements(returning)

    # Sort by arrival time
    sorted_movements = MovementHelper.sort_by_arrival(all_movements)

    # Check if attack is imminent
    is_urgent = MovementHelper.is_attack_imminent(movements_dict, threshold_seconds=60)
