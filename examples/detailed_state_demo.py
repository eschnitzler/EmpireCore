    ...
    try:
        ...
        for castle_id, castle in player.castles.items():
            ...
            if res.iron > 0 or res.glass > 0 or res.ash > 0:
                logger.info(f"\n   ⚙️  Special Resources:")
                if res.iron > 0: logger.info(f"      Iron: {res.iron:,}")
                if res.glass > 0: logger.info(f"      Glass: {res.glass:,}")
                if res.ash > 0: logger.info(f"      Ash: {res.ash:,}")
        ...
        if len(client.state.movements) == 0:
            ...
            for mid, movement in client.state.movements.items():
                ...
                logger.info(f"   Progress: {movement.progress_time}/{movement.total_time}s ({movement.progress_percent:.1f}%)")
                ...
                if movement.is_incoming:
                    logger.info(f"   ⚠️  INCOMING to your area!")
                elif movement.is_outgoing:
                    logger.info(f"   ⏩ OUTGOING from your area")
                else:
                    logger.info(f"   ↩️  RETURNING to your area")
