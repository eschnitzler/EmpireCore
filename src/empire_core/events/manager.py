...
import logging
import inspect
from typing import Callable, Type, Dict, List, Any, Awaitable, Union, get_type_hints
...
class EventManager:
    ...
    def listen(self, func: EventHandler):
        ...
        if event_arg.annotation is inspect.Parameter.empty:
             raise ValueError(f"Handler {func.__name__} argument '{event_arg.name}' must have a type hint (e.g. 'event: AttackEvent').")
