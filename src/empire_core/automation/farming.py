...
class FarmingBot:
    ...
    async def _farm_cycle(self):
        ...
        logger.info(
            f"Attacking target at ({target.x}, {target.y}), distance: {distance:.1f}"
        )
...
class ResourceCollector:
    ...
    async def _collect_cycle(self):
        ...
        for castle_id, castle in player.castles.items():
            ...
            wood_pct = (
                (castle.resources.wood / castle.resources.wood_cap * 100)
                if castle.resources.wood_cap > 0
                else 0
            )
            stone_pct = (
                (castle.resources.stone / castle.resources.stone_cap * 100)
                if castle.resources.stone_cap > 0
                else 0
            )
            food_pct = (
                (castle.resources.food / castle.resources.food_cap * 100)
                if castle.resources.food_cap > 0
                else 0
            )

            if wood_pct > 90 or stone_pct > 90 or food_pct > 90:
                logger.info(
                    f"Castle {castle_id} resources high: W:{wood_pct:.0f}% S:{stone_pct:.0f}% F:{food_pct:.0f}%"
                )
